...
import "./ChapterSingle.css";
import veseImg from "../../assets/ayah1.png";
import { chapterActions } from "../../store";
...
const ChapterSingle = () => {
  ...
  const chapterAudio = useSelector((state) => {
    return state.sora.chapterAudio;
  });
  ...
  useEffect(() => {
    dispatch(chapterActions.getChapterId(chapterID));
  }, [dispatch, chapterID]);
  const [isActive, setIsActive] = useState("reading");
  ...
  const Reading = () => {
    let pagesArr = [];
    for (let i = chapterInfo.startPage; i <= chapterInfo.endPage; i++) {
      pagesArr.push(i);
    }
    return (
      <Fragment>
        {pagesArr?.map((ele) => {
          return (
            <div key={ele} className="container mt-5">
              <div id={ele} className="mb-5 page-div mx-auto alfatha">
                {allVerses?.map((el) => {
                  if (el.page_number === ele)
                    return (
                      <span key={el.id} className="line text-center">
                        {el.textArabic}
                        <div className="num ">
                          <span className="number">{el.verse_number}</span>
                          <img src={veseImg} alt="..."></img>
                        </div>
                      </span>
                    );
                  else {
                ...
                <div className="page-num" lang="ar">
                  {ele}
                </div>
              </div>
              <hr></hr>
  ...
  const [soura] = chapters.filter((ele) => {
    return ele.number === +chapterID;
  });

  useEffect(() => {
    document.title = `Surah ${soura.englishName}`;
  });
  ///////////
  return (
    <div className="chapter-single pt-5">
      <h1 className="text-center sora-name ff-amiri py-5">
        سُورَةٌ {soura.name}
      </h1>
      <div className="container">
        <div className="row ul-info text-center">
          <div className="col-md-6 col-lg-3">
            Located in Juz <span>{allVerses[0]?.juz_number}</span> / Hizb{" "}
            <span>{allVerses[0]?.hizb_number}</span>
          </div>
          <div className="col-md-6 col-lg-3">
            Number of Pages: <span>{chapterInfo.pagesNum + 1}</span>
          </div>
          <div className="col-md-6 col-lg-3">
            Number of Ayahs: <span>{chapterInfo.ayahs}</span>
          </div>
          <div className="col-md-6 col-lg-3">
            Revelation Place: <span>{chapterInfo.revelation_place}</span>
          </div>
        </div>
      </div>
      <div className="audio-div">
        <audio
          src={chapterAudio.url}
          type={`audio/${chapterAudio.format}`}
          controls
        />
      </div>
      <div className="container">
        <div className="row text-center w-50 mx-auto my-5">
          <div className="col-6 ">
            <button
              id="reading"
              className={
                isActive === "reading"
                  ? "btn btn-outline-info w-100 active read"
                  : "btn btn-outline-info w-100 read"
              }
      ...
      {bsmala}
      {isActive === "reading" && <Reading />}
      {isActive === "translation" && <Verses />}
    </div>
  );
};
...
///AbdulBaset/Mujawwad/mp3/001001.mp3
